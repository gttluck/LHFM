...
# CrossScan 只支持 单方向扫描，即水平方向。
# CrossMerge 是将 CrossScan 输出的结果进行合并，但是只处理了单方向的扫描结果。
...
# Improve CrossScan
# 增加了四个方向的扫描：水平前向、垂直前向、水平反向、垂直反向扫描。每个方向都被独立处理，以避免单一方向的扫描造成的信息丢失。
# 为什么重要：在低光图像或有模糊的图像中，图像的各个方向可能包含不同的细节或模糊信息，多个方向的扫描可以帮助捕获更多的全局特征。
# MScan-VSSM (多扫描视觉状态空间模块)

class CrossScan(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape
        ctx.shape = (B, C, H, W)  # 通过将图像从 [B, C, H, W] 变换为 [B, 4, C, H * W]，并在四个方向上进行扫描（水平前向、垂直前向、水平反向、垂直反向），最终生成四个方向的特征图。
        
        # 创建一个新的 tensor 来存储四个方向的扫描结果
        xs = x.new_empty((B, 4, C, H * W))
...
class CrossMerge(torch.autograd.Function):
    ...
    def forward(ctx, ys: torch.Tensor):
        ...
        # 合并操作：前两个方向的特征相加，后两个方向的特征反转后与前面相加
        ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1]).view(B, 2, D, -1)
...
class LocalFeatureModule(nn.Module):
    def __init__(self, dim, growth_rate=2.0):
        ...
        # 亮度感知路径（简化版）
        self.brightness_conv = nn.Sequential(
    ...
    def forward(self, x):
        ...
        ca = self.channel_attn(avg)            # [B,C,1,1]
