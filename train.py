...
import sys
sys.path.append('/home/JJiangx/Documents/gt/LIEDNet/basicsr/archs')
...
# nohup python train.py --opt /home/JJiangx/Documents/gt/LIE/options/train_LOLBlur.yml > training.log 2>&1 &

# ignore UserWarning: Detected call of `lr_scheduler.step()` before `optimizer.step()`.
warnings.filterwarnings("ignore", category=UserWarning)

def run_inference_and_evaluation(opt, current_iter):
    val_opt = opt['datasets']['val']
    test_path = val_opt['dataroot_lq']
    gt_path = val_opt['dataroot_gt']

    # 路径拼接清晰可读
    result_root = osp.join(opt['path']['visualization'], f"{opt['name']}_iter{current_iter}")
    result_dir = osp.join(result_root, osp.basename(test_path))  # 图像最终保存位置
    ckpt_path = osp.join(opt['path']['models'], f'net_g_{current_iter}.pth')

    inference_cmd = [
        sys.executable,
        'inference_lol_blur.py',
        '--test_path', test_path,
        '--result_path', result_root,
        '--ckpt', ckpt_path
    ]
    print(f"\n Running inference for iter {current_iter}")
    ...
    print(f" Running evaluation for iter {current_iter}")
...
def parse_options(root_path, is_train=True):
    ...
    parser.add_argument('--opt', type=str, required=True, default='/home/JJiangx/Documents/gt/LIEDNet/options/train_LOLBlur.yml',help='Path to option YAML file.')
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and opt['datasets'].get('val') is not None \
                ...
                run_inference_and_evaluation(opt,current_iter)
                logger.info('Saving Images and calcute metrics.')
